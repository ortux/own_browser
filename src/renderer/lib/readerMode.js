...
(() => {
  ...
  if (window[FLAG]) {
    ...
    if (snap) {
      document.documentElement.innerHTML = snap.html;
      document.documentElement.style.cssText = snap.docStyle;
  ...
  window[SNAP] = {
    ...
    docStyle: document.documentElement.style.cssText,
  ...
  `;
  document.body.className = 'zr-root';
  ...
  `;
  document.querySelector('.zr-body').appendChild(article);

  let scale = 1;
  const wrap = document.querySelector('.zr-wrap');
  function applyScale() { wrap.style.fontSize = (19 * scale) + 'px'; }
  document.querySelector('[data-zr-size="+"]').onclick = () => { scale = Math.min(1.8, scale + 0.1); applyScale(); };
  document.querySelector('[data-zr-size="-"]').onclick = () => { scale = Math.max(0.7, scale - 0.1); applyScale(); };
  document.querySelector('[data-zr-close]').onclick = () => {
    const snap = window[SNAP];
    if (snap) {
      document.documentElement.innerHTML = snap.html;
      document.documentElement.style.cssText = snap.docStyle;
